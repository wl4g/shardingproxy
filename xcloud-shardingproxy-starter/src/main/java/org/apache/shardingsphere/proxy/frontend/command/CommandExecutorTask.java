...
public final class CommandExecutorTask implements Runnable {
    //
    // ADD feature saving SQL to current thread for admission controller.
    //
    public static final ThreadLocal<String> sqlCaching = new ThreadLocal<>();
     ...
     * ShardingSphere, a new plugin should be provided to SkyWalking project if
     * this API changed.
     *
     * @see <a href=
     *      "https://github.com/apache/skywalking/blob/master/docs/en/guides/Java-Plugin-Development-Guide.md#user-content-plugin-development-guide">Plugin
     *      Development Guide</a>
     */
    @Override
    public void run() {
        ...
        try (PacketPayload payload = databaseProtocolFrontendEngine.getCodecEngine().createPacketPayload((ByteBuf) message,
            ...
            try {
                ...
                exceptions = ex.getExceptions().stream().filter(SQLException.class::isInstance).map(SQLException.class::cast)
    ...
    private boolean executeCommand(final ChannelHandlerContext context, final PacketPayload payload) throws SQLException {
        ...
        if (commandPacket instanceof MySQLComQueryPacket) {
            sqlCaching.set(((MySQLComQueryPacket) commandPacket).getSql());
        } else if (commandPacket instanceof PostgreSQLComQueryPacket) {
            sqlCaching.set(((PostgreSQLComQueryPacket) commandPacket).getSql());
        }
        ...
        try {
            ...
            if (commandExecutor instanceof QueryCommandExecutor) {
                return commandExecuteEngine.writeQueryData(context, connectionSession.getBackendConnection(),
    ...
    private void processException(final Exception cause) {
        ...
        Optional<DatabasePacket<?>> databasePacket = databaseProtocolFrontendEngine.getCommandExecuteEngine()
                .getOtherPacket(connectionSession);
