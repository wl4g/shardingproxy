...
public final class AuthorityChecker implements SQLChecker<AuthorityRule> {
    ...
    public SQLCheckResult check(final SQLStatement sqlStatement, final List<Object> parameters, final Grantee grantee,
            final String currentSchema, final Map<String, ShardingSphereMetaData> metaDataMap,
        ...
        if (!privileges.isPresent()) {
            return new SQLCheckResult(false,
                    String.format("Access denied for user '%s'@'%s'", grantee.getUsername(), grantee.getHostname()));
        }
        ...
        //
        // ADD feature for SQL statement admission control
        //
        SQLCheckResult admission = doSQLAdmission(sqlStatement, grantee, authorityRule);
        if (!admission.isPassed()) {
            return admission;
        }
        ...
        return privileges.map(
                optional -> new SQLCheckResult(optional.hasPrivileges(Collections.singletonList(privilegeType)), errorMessage))
